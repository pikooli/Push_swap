...
char *ft_algo_a(t_list *a, t_list *b, t_option *option)
{
    ...
    else if (a->begin && a->begin->next && a->begin->val < a->begin->next->val &&
            (!b->begin || b->begin->val < a->begin->val))
                return ft_pb(a, b, option);
    else if (ft_check_rev_list(b) && a->begin && a->begin->next && a->begin->val < b->begin->val &&
            a->begin->next->val > b->begin->val)
                return ft_pa(a, b, option);
    else if (ft_check_rev_list(b) && a->begin && a->begin->next && a->begin->val < b->begin->val &&
            a->begin->next->val < b->begin->val)
                return ft_ra(a, option);
    else if (a->begin && !ft_check_list(a) && a->begin->next &&  a->begin->val > a->begin->next->val)
                return ft_ra(a, option);
